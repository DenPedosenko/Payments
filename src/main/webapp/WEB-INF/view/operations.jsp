	...
	<div class="container-fluid">
		...
		<div class="container">
			<table class="table table-sm align-middle mt-2">
				<thead>
					<tr>
						<th scope="col" role="button" onclick="document.location = '<%out.print(request.getContextPath());%>/operations?language=<% out.print(language); %>&orderBy=id&ascending=<%= orderBy.equals("date")||orderBy.equals("paymentStatus")?"true":ascending.equals("true")?"false":"true" %>';">
						<fmt:message key="operations.number"/>
						<c:choose>
						...
						<th scope="col" role="button" onclick="document.location = '<%out.print(request.getContextPath());%>/operations?language=<% out.print(language); %>&orderBy=date&ascending=<%= orderBy.equals("id")||orderBy.equals("paymentStatus")?"true":ascending.equals("true")?"false":"true" %>';">
						<fmt:message key="operations.date"/>
						<c:choose>
						...
						<th scope="col"><fmt:message key="operations.account"/></th>
						<th scope="col"><fmt:message key="operations.payment_type"/></th>
						<th scope="col" role="button" onclick="document.location = '<%out.print(request.getContextPath());%>/operations?language=<% out.print(language); %>&orderBy=paymentStatus&ascending=<%= orderBy.equals("id")||orderBy.equals("date")?"true":ascending.equals("true")?"false":"true" %>';">
						<fmt:message key="operations.payment_status"/>
						<c:choose>
								<c:when test="${orderBy == 'paymentStatus' && ascending=='true'}">
									<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
										fill="currentColor" class="bi bi-arrow-up" viewBox="0 0 16 16">
  										<path fill-rule="evenodd"
											d="M8 15a.5.5 0 0 0 .5-.5V2.707l3.146 3.147a.5.5 0 0 0 .708-.708l-4-4a.5.5 0 0 0-.708 0l-4 4a.5.5 0 1 0 .708.708L7.5 2.707V14.5a.5.5 0 0 0 .5.5z" />
								...
								<c:when test="${orderBy == 'paymentStatus' && ascending=='false'}">
									<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-arrow-down" viewBox="0 0 16 16">
  										<path fill-rule="evenodd" d="M8 1a.5.5 0 0 1 .5.5v11.793l3.146-3.147a.5.5 0 0 1 .708.708l-4 4a.5.5 0 0 1-.708 0l-4-4a.5.5 0 0 1 .708-.708L7.5 13.293V1.5A.5.5 0 0 1 8 1z"/>
									</svg>
								</c:when>
						...
						<th scope="col"><fmt:message key="operations.amount"/></th>
					</tr>
				</thead>
				<tbody>
				<c:forEach var="operation" items="${operations}">
					<tr>
						...
						<td>${DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss").format(operation.getCreationDate())}</td>
						<td>${operation.getUserAccount().getName()}</td>
